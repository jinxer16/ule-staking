...
export default function Stacking_With_100() {
  ...
  //       toast.info("Wallet balance insufficient!!!");
  ...
  //       toast.info("Enter Minimum package amount 100 USD!!!");
  //       setLoadingTrans(false);
  //       return;
  //     }
  //     if (parseInt(usdamt) < 100) {
  ...
  //       toast.info("Enter package amount in multiple of 100 USD!!!");
  //       setLoadingTrans(false);
  //       return;
  //     }

  //     if (parseInt(usdamt) > 10000) {
  //       toast.info("Maximum package amount is 10000 USD");
  //       setLoadingTrans(false);
  //       return false;
  //     }
  ...
  const ULE_Stake = async () => {
    ...
    try {
      ...
      console.log("userInfo.ress", ress, uId_user);
          ...
          if (check_Nft_Balance == acc) {
            if (Api_Conditon == "Success") {
              ...
              let hash = await ULE_Staking_ContractOf.methods
                .Stake([tokenid], ULE_NFT_100)
                .send({
                  from: acc,
                  // value: totalMintingPriceBNB.toString()
                });
              console.log("Stake Stake", hash);
              //
    ...
    try {
      ...
      if (userInfo.EthAddress.toLowerCase() == acc.toLowerCase()) {
        if (tokenid == "") {
          ...
          // console.log("LockedConditon", LockedConditon, check_Nft_Balance, res);
          if (res == tokenid) {
            if (LockedConditon == true) {
              let hash = await ULE_Staking_ContractOf.methods
                .unStake([tokenid], ULE_NFT_100)
                .send({
                  from: acc,
                });
      ...
      console.log("Erroe While Call Un Staking Fuction", error);
      toast.error("Transaction Failed");
      setLoadingTrans(false);
    }
  };
